...
Gradient boosting methods are also popular for community-level biodiversity modeling \parencite{cai2023, baggstrom2025} and also for modeling the value of the ecosystem \parencite{liu_ada-xg-catboost_2024}. However, some limitations of gradient boosting are the conditional shift problem. This is when the distribution of the training data differs from the distribution of the test data \parencite{zhang_domain_nodate, Prokhorenkova2018}. Gradient boosting can also overfit if the learning rate isn't properly tuned \parencite{friedman2001}. Additionally, gradient boosting can be computationally expensive, especially when the number of weak learners \(M\) is large or when the weak learners are complex.
